...
public class ConstantConfig extends Config {
     ...
     * 奖励List
     */
    public final List<quan.config.test.item.Reward> rewardList;
    ...
    public ConstantConfig(JSONObject json) {
        ...
        List<quan.config.test.item.Reward> rewardList$2 = new ArrayList<>();
        if (rewardList$1 != null) {
            for (int i = 0; i < rewardList$1.size(); i++) {
                quan.config.test.item.Reward rewardList$Value = quan.config.test.item.Reward.create(rewardList$1.getJSONObject(i));
